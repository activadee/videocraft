...
func (ts *transcriptionService) transcribeWithDaemon(ctx context.Context, url string) (*TranscriptionResult, error) {
...
func (ts *transcriptionService) ensureDaemon() error {
	ts.mutex.Lock()
...
func (ts *transcriptionService) waitForDaemonReady() error {
...
func (ts *transcriptionService) monitorDaemon() {
	if ts.daemon == nil {
		return
	}
...
func (ts *transcriptionService) logDaemonErrors() {
...
func (ts *transcriptionService) shouldRestartDaemon() bool {
	now := time.Now()
...
func (ts *transcriptionService) stopDaemon() {
	ts.mutex.Lock()
	defer ts.mutex.Unlock()
...
func (ts *transcriptionService) convertToTranscriptionResult(response TranscriptionResponse) *TranscriptionResult {
